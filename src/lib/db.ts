    ...
    if (ole && thea) {
      ...
      const seedSources = db.transaction(() => {
        ...
        ins.run(s1, ole.id, 'Google Calendar', 'ical_url',
          'https://calendar.google.com/calendar/ical/ole.randhendriksen%40gmail.com/public/basic.ics',
          null, null, null);
        insSP.run(s1, ole.id);

        const s2 = uuidv4();
        ins.run(s2, ole.id, 'Jobb (Lillestrøm)', 'ical_url',
          'https://outlook.office365.com/owa/calendar/092f8d7d612643d4b8002d2110cb008a@lillestrom.kommune.no/21ac0cbfd8074dc5a437dd4d577e281b8559839646037177238/S-1-8-3995730387-3462510330-2111249928-1385885730/reachcalendar.ics',
          null, null, null);
        insSP.run(s2, ole.id);

        const s3 = uuidv4();
        ins.run(s3, thea.id, 'Cheerleading', 'ical_url',
          'https://api.hoopit.io/events/ics/d229587a-6c46-4b88-981b-987c5bef0bd1/events.ics',
          null, null, null);
        insSP.run(s3, thea.id);

        const s4 = uuidv4();
        ins.run(s4, thea.id, 'Google Calendar', 'ical_url',
          'https://calendar.google.com/calendar/ical/9duci707qduca65imsqabq7mk%40group.calendar.google.com/public/basic.ics',
          null, null, null);
